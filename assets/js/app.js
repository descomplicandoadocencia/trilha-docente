...
function renderStats() {
  ...
  document.querySelectorAll('[data-progress]').forEach(e => e.textContent = done.length + '/' + VIDEOS.length);
}
...
async function initAula() {
  ...
  if (v.h5p) {
    ...
    hb.innerHTML = `<h2 style="font-size:1.05rem;margin-bottom:8px">🧪 Atividade interativa H5P</h2>
      <iframe src="h5p/player.html?p=${encodeURIComponent(v.h5p)}" title="Atividade H5P" loading="lazy"></iframe>`;
  }

  // Cornell Notes — autosave por vídeo
